...
namespace Vupa
{
    ...
    {
        private Cell[,] colMap;
        private Dictionary<Point, listitem> list = new Dictionary<Point, listitem>();
        public List<listitem> openList { get; set; } = new List<listitem>(); 
        ...
        public A_Star(Cell[,] ColMap, int _hWeight)
        {
            hWeight = _hWeight;
            colMap = ColMap;
        ...
        public bool start(int Sx, int Sy, int Gx, int Gy)
        {
            ...
            if ((Gx - 1 < 0 || colMap[Gx - 1, Gy].cellType == Cell.CellType.Wall) &&
               (Gx + 1 > Width || colMap[Gx + 1, Gy].cellType == Cell.CellType.Wall) &&
                (Gy - 1 < 0 || colMap[Gx, Gy - 1].cellType == Cell.CellType.Wall) &&
                (Gy + 1 > Width || colMap[Gx, Gy + 1].cellType == Cell.CellType.Wall))
                noPath = true;


            if (!(Sx == Gx && Sy == Gy) && colMap[Gx, Gy].cellType != Cell.CellType.Wall && !noPath)
            {
                ...
                //else if (colMap[Gx, Gy].unitOnCell != null && colMap[Gx, Gy].unitOnCell.isPlayer
        ...
        bool isWalkable(int x, int y)
        {
            ...
            if (colMap[x, y].cellType == Cell.CellType.Wall)
            {
            ...
            else if (colMap[x, y].unitOnCell != null && colMap[x, y].unitOnCell.isPlayer && x != goalX && y != goalY)
            {
                tempItem.closed = true;
                if (!list.ContainsKey(Loc))
                    list.Add(Loc, tempItem);
        ...
        //                    Console.Write(" C ");
        //                }
        //                else if (colMap[x, y].cellType == Cell.CellType.Wall)
        //                {
